...
var startSpeed 		= 2;
var endSpeed 		= 0.01;
// add multiple reruns to repeat the grid and try multiple reruns (going over the same grid again)
var numReruns		= 3;
var gridSpace		= 3;

var speedStep	 	= (endSpeed-startSpeed)/(numY-1); // speed over y-axis
var powerStep 		= (endPower-startPower)/(numX-1); // power over x-axis
var gridW			= rectangleWidth*numX+space*(numX-1);

echo('This is a test pattern writen in Laserscript. More info:');
echo('https://github.com/t-oster/VisiCut/wiki/LaserScript. ');
for (var g = 0; g < numReruns; g++) // grid
{
	var startX = g*(gridW+gridSpace);
	if(numReruns > 1) 
	{
		echo(' ');
		echo('Grid:  '+(g+1)+' ('+(g)+' reruns)');
	}
	for (var r = 0; r < g+1; r++) // rerun
	{
		for (var i = 0; i < numY; i++)
		{
			...
			for (var j = 0; j < numX; j++)
			{
				...
				set("power", power);
				var x = startX+(rectangleWidth+space)*j;
